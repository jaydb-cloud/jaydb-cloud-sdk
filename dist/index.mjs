// src/errors.js
var JayDBError = class extends Error {
  /**
...
var ConflictError = class extends JayDBError {
  constructor(message, opts) {
    super(message, opts);
    this.name = "ConflictError";
  }
};
var NotFoundError = class extends JayDBError {
  constructor(message, opts) {
    super(message, opts);
    this.name = "NotFoundError";
  }
};
var AuthError = class extends JayDBError {
  constructor(message, opts) {
    super(message, opts);
    this.name = "AuthError";
  }
...
function encodeKey(key) {
  return String(key).replace(/^\/+|\/+$/g, "").split("/").map(encodeURIComponent).join("/");
}
function unquoteETag(value) {
  if (!value) return null;
  return String(value).replace(/^W\//, "").replace(/^"/, "").replace(/"$/, "");
}
var JayDBClient = class {
  /**
   * @param {object} opts
  ...
  async #request(method, url, { headers = {}, body, signal } = {}, key = null) {
    ...
    try {
      ...
      throw new JayDBError(
        `Network or CORS failure calling ${method} ${url}. If the server is reachable, verify that your application's origin is listed in allowed origins.`,
   ...
   */
  async listAll({ prefix = "", pageLimit = 1e3, signal } = {}) {
    const items = [];
    let cursor;
    do {
...
// src/auth.js
var STORAGE_PREFIX = "jaydb_pkce_";
var VERIFIER_KEY = `${STORAGE_PREFIX}verifier`;
var STATE_KEY = `${STORAGE_PREFIX}state`;
var RETURN_KEY = `${STORAGE_PREFIX}return`;
var TOKENS_KEY = "jaydb_oidc_tokens";
var TOKEN_ENDPOINT_KEY = `${STORAGE_PREFIX}token_endpoint`;
...
var JayDBAuth = class {
  ...
  storeTokens(tokens, tokenEndpoint, clientId) {
    const expiresAt = tokens.expires_in ? Date.now() + tokens.expires_in * 1e3 : 0;
  ...
  async getToken() {
    ...
    const isStale = t.expiresAt && Date.now() > t.expiresAt - 3e4;
...
async function beginLogin(opts) {
  return getDefaultAuth().signIn(opts);
}
async function completeLoginIfCallback(opts) {
...
// src/treeacl.js
var DEFAULT_SECRET = "jaydb-kanban-tree-acl-signing-key-2026";
function getCrypto2() {
  if (typeof globalThis !== "undefined" && globalThis.crypto) {
    return globalThis.crypto;
  }
...
var TreeACL = class {
  ...
  static async signInvite({
    ...
    const cryptoObj = getCrypto2();
    const nonce = b64url2(cryptoObj.getRandomValues(new Uint8Array(12)));
    const payload = {
      type: "tree_invite",
      tree_path: path,
      role,
    ...
    const enc = new TextEncoder();
    const payloadB64 = b64url2(enc.encode(JSON.stringify(payload)));
    const key = await getHmacKey(secret);
    const sigBuffer = await cryptoObj.subtle.sign("HMAC", key, enc.encode(payloadB64));
    const sigB64 = b64url2(sigBuffer);
...
async function signBoardInvite(opts) {
  return TreeACL.signInvite(opts);
}
async function verifyBoardInvite(rawToken, opts) {
...
//# sourceMappingURL=jaydb-cloud.esm.js.map
